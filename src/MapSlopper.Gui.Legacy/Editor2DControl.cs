...
public class Editor2DControl : Control
{
    ...
    public Editor2DControl()
    {
        ...
        // Avalonia 0.10: use GetObservable(BoundsProperty) for size changes.
        this.GetObservable(BoundsProperty).Subscribe(new BoundsObserver(this));
    }

    private sealed class BoundsObserver : IObserver<Rect>
    {
        private readonly Editor2DControl _owner;
        public BoundsObserver(Editor2DControl owner) => _owner = owner;
        public void OnNext(Rect value)
        {
            if (!_owner._userHasModifiedCamera && value.Width > 50 && value.Height > 50)
                _owner.FrameProject();
    ...
    public void FrameProject()
    {
        ...
        PixelsPerWorldUnit = Math.Max(0.01, Math.Min(zoomX, zoomY) * 0.85);
        _vm.PixelsPerWorldUnit = PixelsPerWorldUnit;
    ...
    protected override void OnPointerPressed(PointerPressedEventArgs e)
    {
        ...
        {
            _isLeftDown = true;
            _vm.ShiftDown = (e.KeyModifiers & KeyModifiers.Shift) != 0;
        ...
        for (var y = cy0; y <= cy1; y++)
        {
            for (var x = cx0; x <= cx1; x++)
            {
                ...
                if (brush is null)
                {
                    ...
                    var rgb = TriggerBrushTool.ParseColor(t.ColorHex);
    ...
    private void DrawPlayerReference(DrawingContext ctx)
    {
        ...
        var labelBrush = Brushes.LightSkyBlue;
        var origin = WorldToScreen(new Vec2(-16, -16));
        ctx.DrawText(labelBrush, new Point(origin.X, origin.Y + 2), label);
    }

    /// <summary>
    /// Build a small label FormattedText compatible with Avalonia 0.10.
    /// In 0.10, brush is passed to DrawingContext.DrawText, not to FormattedText.
    /// </summary>
